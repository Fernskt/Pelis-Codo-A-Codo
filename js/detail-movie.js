document.addEventListener("DOMContentLoaded", async function () {
  ...
  const displayMovieDetails = (movie) => {
    ...
    <p id="pais"><span class="enfasis">País: </span>${movie.production_countries[0].name}</p>
        <p id="director"><span class="enfasis">Compañía: </span>${movie.production_companies[0].name}</p>`;
  };

  const displayTrailers = (videosData) => {
    // Suponiendo que quieres el primer trailer disponible
